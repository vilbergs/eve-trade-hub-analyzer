...
use eve_market::{RunSummary, delete_region_current, ensure_partitions, filter_by_type, ingest_orders};
...
async fn poll_jita_inner(
    ...
    delete_region_current(pool, region_id).await?;
    for (location_id, orders) in by_loc {
        ingest_orders(pool, location_id, Some(region_id), &orders, observed_at).await?;
    }
